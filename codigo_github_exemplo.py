...
@connector.ready
async def pick_champ(connection):
                    ...
                    for event in event_group:
                        if event["actorCellId"] == position and event["type"] == "pick" and event["isInProgress"]:
                            ...
                            if "internalNowInEpochMs" in timer_info:
                                ...
                                print("Detected pick turn. Sleeping " + str(remaining - 1))
                                time.sleep(remaining - 1)
                                lock_in = await connection.request('patch', '/lol-champ-select/v1/session/actions/'+ str(event_id), data = {"championId": 103, "completed" : True})
